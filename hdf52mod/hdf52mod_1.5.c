 ...
 *             25 Apr 2025
 *   Ver 1.3   Hybrid Task Info added to be copied from original RAW
 *             30 Nov 2025                    
 *   Ver 1.4   Error correction for Hybrid Ingest
 *             05 Dec 2025          
 *   Ver 1.5   Header info filled for ingest error correction
 *              - melting height / H-beamwidth / V-beamwidth
 *             13.Dec.2025                          
 ******************************************************************************/
...
/* ----------------------------  */
		...
		for (i=0;i<nblk;i++){
			...
			/* -------------------------------*/
				buf[266]=bufr[266];     /* missing Melting Level Info to be filled */
                                buf[267]=bufr[267];
			}
			...
			if (i==1){
				...
				buf[1426]=bufr[1426];   /* AZ Resolution*/
				buf[1427]=bufr[1427];
		       /* -------------------------------*/
				...
				if(atoi(debug)==1)printf("ZCAL H5RAW= %f, orgRAW= %f ",h5cal, orcal);
                                buf[3908]=bufr[3908];   /* ZCAL */
                                buf[3909]=bufr[3909];
                       /* ------------------------------*/
				buf[2064]=bufr[2064];   /* Hybrid Task Info */
				buf[2066]=bufr[2066];
				buf[2160]=bufr[2160];
                       /* ------------------------------*/
				buf[192]=bufr[192];     /* missing Sector resolution to be filled */
				buf[193]=bufr[193];
                       /* -------------------------------*/
                                buf[232]=bufr[232];     /* missing Melting Level Info to be filled */
                                buf[233]=bufr[233];
                       /* -------------------------------*/
                                for(k=0;k<8;k++)buf[1808+k]=bufr[1808+k];     /* missing H&V Beamwidth Info to be filled */
			}
